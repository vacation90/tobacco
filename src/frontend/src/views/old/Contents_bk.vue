<template>
  <!-- Contents -->
  <div>
    <va-button-toggle style="width: fit-content" class="ma-3"
    :rounded="false"
    v-model="model"
    :options="[
      { label: 'One', value: 'one' },
    ...
    <div class="row">
    <div>
      <template v-for="data in models.data" :key="data.to">
        <div class="flex xs12 sm6 lg4 xl3 ma-3" style="width: 100%;">
        <va-card stripe stripe-color="info"
          @click="showFirstModal = !showFirstModal">
          <!-- Image -->
          <va-image
              :src="data.image"
              style="height: 200px;"/>
            ...
            <!-- Disc -->
            <va-card-content class="card-disc">
              {{data.discription}}
            </va-card-content>
        </va-card>
      </div>
...
const baseURL = 'http://localhost:8000/items/all';
...
export default defineComponent({
  ...
  setup() {
    ...
    const getModel = () => {// eslint-disable-line no-unused-vars
      axios.get(baseURL).then((res) => {
        if (res && res.data) {
          console.log(res.data);
          models.data = res.data;
          //models.title = res.data[0].name;
          //models.dist = res.data[0].code;
        }
      });
    };
    getModel();
